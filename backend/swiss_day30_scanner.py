"""
Swiss Day=30 — every historical Swiss draw on day=30 of any month
with the BD (previous draw) for clue mining.

Tomorrow Wed 03.06.2026 is day=30 — look for grammar.
"""
...
async def main():
    ...
    day3 = [(i, r) for i, r in enumerate(rows) if r["dt"].day == 30]
    print(f"📊 Total Swiss day=30 draws: {len(day3)}\n")
    print("=" * 110)
    print("🎯 EVERY Swiss day=30 + BD pair")
    print("=" * 110)
    print(f"{'BD':<13} {'BD-mains':<28} {'BD🍀':<6} → {'ND (day=30)':<13} {'ND-mains':<28} {'ND🍀':<6}")
    print("-" * 110)
    ...
    same_month = 0       # ND day=30 + BD same month
    ...
    # Filter: cross-month day=30 only (matches tomorrow's situation)
    print()
    print("=" * 110)
